...
ASTNode *ParseStatement(FILE *file, bool isExpectingElif) {
  ...
  if (token.type == IfToken) {
    ...
    ASTNode *conditionEvalNode = CreateNode(BlockNode, "ConditionEvaluator");
          ...
          if (nextToken.type == ElifToken) {
            ...
            if (checkOpenCurly.type == OpenCurlyBracket) {
              ...
              while (true) {
                ...
                if (currentLine != NULL) {
                  elseNode->children[elseNode->childCount] = currentLine;
                  elseNode->childCount++;
      ...
      if (closeParenToken.type == CloseParenthesisToken) {
        ...
        if (openCurlyTok.type == OpenCurlyBracket) {
          ...
          while (true) {
            ...
            if (lineTok.type == CloseCurlyBracket || lineTok.type == EOFToken) {
              break;
            }
            ...
            if (currentLine != NULL) {
              elifNode->children[elifNode->childCount] = currentLine;
              elifNode->childCount++;
            } else {
              ConsumeToken(file);
            }
  ...
  if (token.type == KeyWordToken &&
      (strcmp(token.value, "int") == 0 || strcmp(token.value, "string") == 0 ||
  ...
  if (token.type == KeyWordToken && strcmp(token.value, "func") == 0) {
    ConsumeToken(file);
    Token nextToken = PeekToken(file);

    if (nextToken.type == FunctionDeclToken) {
      ConsumeToken(file);
      ASTNode *functionNode = CreateNode(FunctionDeclNode, nextToken.value);
      functionNode->left = NULL;
      Token openParenToken = PeekToken(file);
      ...
      if (openParenToken.type == OpenParenthesisToken) {
        ...
        if (nextToken.type == CloseParenthesisToken) {
          ConsumeToken(file);
          ...
          if (openCurlyTok.type == OpenCurlyBracket) {
            ...
            ASTNode *functionBlockNode = CreateNode(BlockNode, "BlockNode");

            while (true) {
              Token closeCurlyTok = PeekToken(file);

              if (closeCurlyTok.type == CloseCurlyBracket ||
                  closeCurlyTok.type == EOFToken) {
                break;
              }
              ...
              if (currentLine != NULL) {
                functionBlockNode->children[functionBlockNode->childCount] =
                    currentLine;
                functionBlockNode->childCount++;
              } else {
                ConsumeToken(file);
              }
            }
            ConsumeToken(file);
            functionNode->right = functionBlockNode;
          }
        } else {
          ConsumeToken(file);
